...
#Author: gustavooliveira@ext.cvccorp.com.br
 ...
 @FlightAvailability @Cenário03 @Regressivo
Funcionalidade: Validação de get em FlightAvailability resposta negativa
  ...
  Cenário: Get com dez adultos
   ...
   Cenário: Get sem passageiro
    Dado que o usuário faça uma pesquisa de disponibilidade de voo "https://search-cvc-hom.reservafacil.tur.br/gwaereo/v0/flights?ages=&preferences=persistLog,language:pt_BR,currency:BRL,maxResults:600&source=NLAT&routes=SAO,RIO,2024-02-02&packageGroup=GW-CERT&economyClass=YES&economyPremiumClass=YES"
    Quando preencho uma váriavel negativa para consultar disponibilidade
    Então válido statusCode 500
    E válido código de erro "05-001-000" e resposta "[Dados de entrada inconsistentes, 1673 - Number of passengers exceeded the limit]"
   ...
   Cenário: Get sem areporto 
    Dado que o usuário faça uma pesquisa de disponibilidade de voo "https://search-cvc-hom.reservafacil.tur.br/gwaereo/v0/flights?ages=30&preferences=persistLog,language:pt_BR,currency:BRL,maxResults:600&source=NLAT&routes=RIO,2024-02-02&packageGroup=GW-CERT&economyClass=YES&economyPremiumClass=YES"
    Quando preencho uma váriavel negativa para consultar disponibilidade
    Então válido statusCode 500
    
   @CT04
   Cenário: Get com aeroporto inválido (SAMP)
    Dado que o usuário faça uma pesquisa de disponibilidade de voo "https://search-cvc-hom.reservafacil.tur.br/gwaereo/v0/flights?ages=30&preferences=persistLog,language:pt_BR,currency:BRL,maxResults:600&source=NLAT&routes=SAMP,RIO,2024-02-02&packageGroup=GW-CERT&economyClass=YES&economyPremiumClass=YES"
    Quando preencho uma váriavel negativa para consultar disponibilidade
    Então válido statusCode 500
